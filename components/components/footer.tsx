...
import { brandContact, brandSocialLinks } from '@/lib/brand-contact';
...
function FooterLink({ href, locale, target, children }: { href: string; locale: Locale; target?: string; children: React.ReactNode }) {
  ...
  const className = 'group/footer-link flex min-h-[32px] items-center justify-between gap-2 rounded-full border border-transparent px-2.5 text-[13.5px] font-bold tracking-[-0.018em] text-pearl/64 transition duration-300 ease-luxe hover:translate-x-1 hover:border-pearl/12 hover:bg-pearl/[0.07] hover:text-gold lg:min-h-[38px] lg:px-3 lg:text-[14.5px]';
...
function PaymentBadge({ label }: { label: (typeof paymentBadges)[number] }) {
  return (
    <span
      ...
      className="group/pay inline-flex h-[34px] min-w-[62px] items-center justify-center rounded-[8px] border border-pearl/50 bg-[#eef6ff] px-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.82),0_6px_12px_rgba(0,0,0,0.10)] transition duration-300 ease-luxe hover:-translate-y-0.5 hover:border-gold/70 hover:bg-pearl hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.92),0_10px_20px_rgba(200,169,106,0.14)]"
    >
      {label === 'VietQR' && (
      ...
      {label === 'NAPAS' && (
        <span className="relative inline-flex min-w-[52px] items-center justify-center overflow-hidden rounded-[4px] bg-white px-1.5 py-[0.42rem] text-[10px] font-black uppercase italic tracking-[-0.08em] text-[#0a4c98] ring-1 ring-[#dbe8f7]">
          NAPAS
      ...
      {label === 'VNPAY' && (
        <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase leading-none tracking-[-0.08em]">
          <span className="rounded-[3px] bg-[#005baa] px-1.5 py-[0.32rem] text-white">VN</span>
          <span className="rounded-[3px] bg-[#e31b23] px-1.5 py-[0.32rem] text-white">PAY</span>
        </span>
      )}
      {label === 'Visa' && (
...
export function Footer({
  ...
  const footer = {
    ...siteContent.footer,
    phoneHref: brandContact.phoneHref,
    phoneDisplay: brandContact.phoneDisplay,
  ...
  return (
    <footer className="relative w-full overflow-hidden bg-navy text-pearl border-t border-gold/15">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_12%_0%,rgba(200,169,106,0.15),transparent_40%),radial-gradient(circle_at_88%_78%,rgba(248,245,239,0.05),transparent_30%)]" />
      <div className="pointer-events-none absolute inset-0 opacity-[0.04] [background-image:linear-gradient(rgba(248,245,239,0.16)_1px,transparent_1px),linear-gradient(90deg,rgba(248,245,239,0.12)_1px,transparent_1px)] [background-size:52px_52px]" />
      ...
      <div className="relative mx-auto max-w-[1680px] px-[clamp(20px,4vw,80px)] py-[clamp(32px,4vw,64px)]">
        <section aria-label="Travel partners and accreditations" className="relative grid justify-items-center gap-x-[clamp(20px,3.5vw,48px)] gap-y-6 border-b border-pearl/18 pb-[clamp(24px,3.5vw,48px)] sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-[repeat(7,minmax(100px,1fr))]">
          {awardMarks.slice(0, 1).map((mark) => {
            const Icon = mark.icon;

            return (
              <div key={mark.title} className="group/award flex min-h-[82px] w-full flex-col items-center justify-center text-center opacity-72 transition duration-500 ease-luxe hover:-translate-y-1 hover:opacity-100">
                <Icon className="h-9 w-9 stroke-[1.4] text-gold transition duration-500 ease-luxe group-hover/award:scale-110 group-hover/award:drop-shadow-[0_10px_22px_rgba(200,169,106,0.34)]" />
                <p className="mt-2 text-[11px] font-black uppercase leading-4 tracking-[0.1em] text-pearl/78 transition duration-300 group-hover/award:text-gold">{mark.title}</p>
                <p className="mt-1 text-[10px] font-bold text-pearl/42 transition duration-300 group-hover/award:text-pearl/68">{mark.note}</p>
              </div>
            );
          })}

          {partnerMarks.map((mark) => (
            <div data-footer-partner-mark key={mark.name} className="group/logo flex min-h-[82px] w-full flex-col items-center justify-center text-center opacity-62 transition duration-500 ease-luxe hover:-translate-y-1 hover:opacity-100">
              <Image
                src={mark.src}
                alt={`${mark.name} logo`}
                width={mark.width}
                height={mark.height}
                className="max-h-[50px] w-auto object-contain grayscale brightness-150 contrast-75 transition duration-500 ease-luxe group-hover/logo:scale-[1.08] group-hover/logo:grayscale-0 group-hover/logo:brightness-110 group-hover/logo:contrast-100 group-hover/logo:drop-shadow-[0_12px_24px_rgba(200,169,106,0.22)]"
              />
              <p className="mt-2 text-[10px] font-bold text-pearl/42 transition duration-300 group-hover/logo:text-pearl/72">{mark.note}</p>
            </div>
          ))}
          ...
          {awardMarks.slice(1).map((mark) => {
            ...
            return (
              <div key={mark.title} className="group/award flex min-h-[82px] w-full flex-col items-center justify-center text-center opacity-72 transition duration-500 ease-luxe hover:-translate-y-1 hover:opacity-100">
                <Icon className="h-9 w-9 stroke-[1.4] text-pearl/50 transition duration-500 ease-luxe group-hover/award:scale-110 group-hover/award:text-gold group-hover/award:drop-shadow-[0_10px_22px_rgba(200,169,106,0.34)]" />
                <p className="mt-2 text-[18px] font-black uppercase leading-none tracking-[-0.04em] text-pearl/62 transition duration-300 group-hover/award:text-gold">{mark.title}</p>
                <p className="mt-1 text-[10px] font-bold text-pearl/42 transition duration-300 group-hover/award:text-pearl/68">{mark.note}</p>
              </div>
            );
          })}
        </section>
        <div className="relative mt-[clamp(24px,3.5vw,48px)] grid gap-x-6 gap-y-10 border-b border-pearl/18 pb-[clamp(28px,4vw,56px)] grid-cols-2 lg:grid-cols-[1.18fr_0.86fr_0.86fr_0.86fr] xl:gap-x-[clamp(32px,5vw,96px)]">
          <section aria-labelledby="footer-contact" className="col-span-2 lg:col-span-1 p-[clamp(10px,2vw,22px)]">
            <p id="footer-contact" className="text-[clamp(18px,1.2vw,22px)] font-black tracking-[-0.045em] text-white">{footer.contactHeading}</p>
          ...
          {footerColumns.map((column) => {
            ...
            const navLayout = isDestinationColumn
              ? 'mt-4 grid gap-x-2 gap-y-1 xs:grid-cols-2 xl:gap-x-4'
              : 'mt-4 grid gap-1';

            return (
              <section key={column.title} aria-labelledby={columnId} className="col-span-1 min-w-0">
                <p id={columnId} className="text-[clamp(16px,1.1vw,20px)] font-black tracking-[-0.04em] text-white">{column.title}</p>
        ...
        <div className="relative grid gap-[clamp(20px,3vw,44px)] pt-[clamp(28px,4vw,56px)] lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div>
            <BrandLogo variant="footer" identity={siteContent.identity} />
            <p className="mt-6 max-w-[62rem] text-[13.5px] font-bold leading-7 text-pearl/50">
              {footer.copyright}
            </p>
          </div>

          <div className="grid gap-6 lg:min-w-[min(760px,54vw)]">
            <div className="grid gap-2.5 lg:justify-items-end">
              <div className="flex items-center gap-1.5 text-[13.5px] font-black tracking-[-0.02em] text-white">
                <ShieldCheck className="h-4 w-4 fill-[#4fd5c8]/20 text-[#4fd5c8]" />
                Secured Payment:
              </div>
              <div className="flex flex-wrap items-center gap-x-2.5 gap-y-3.5 lg:justify-end">
